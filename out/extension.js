...
function getRegisterCategory(reg) {
    if (reg.match(/^%(r(ax|bx|cx|dx|si|di|bp|sp|ip)|r(8|9|1[0-5]))$/))
        return 'gpr';
    if (reg.match(/^%(e(ax|bx|cx|dx|si|di|bp|sp)|r(8|9|1[0-5])d)$/))
        return 'gpr';
    if (reg.match(/^%((ax|bx|cx|dx|si|di|bp|sp)|r(8|9|1[0-5])w)$/))
        return 'gpr';
    if (reg.match(/^%([abcd][hl]|[sd]il|[sb]pl|r(8|9|1[0-5])b)$/))
        return 'gpr';
...
function setRegister(state, rawReg, value) {
...
function detectDeadCode(document) {
    ...
    const branchTargetLabels = new Set();
    for (let i = 0; i < document.lineCount; i++) {
    ...
    function flushDeadCode(firstLine, lastLine, reason) {
        if (firstLine !== -1 && lastLine !== -1) {
            deadCodeRanges.push({
                range: new vscode.Range(firstLine, 0, lastLine, document.lineAt(lastLine).text.length),
                reason
            });
        }
    }
...
class RegisterStateProvider {
    ...
    getChildren(element) {
        ...
        if (!element) {
            ...
            const isFrameSlotAddr = (a) => a.includes('%rbp') || a.includes('%rsp');
            ...
            // Memory
            const isFrameSlot = (addr) => addr.includes('%rbp') || addr.includes('%rsp');
            ...
            if (element.label?.startsWith('General Purpose Registers')) {
                ...
                for (const [flag, afterCondition] of Object.entries(this.stateAfter.flags)) {
                    ...
                    if (changed) {
                        ...
                        items.push(item);
                        const afterItem = new RegisterTreeItem(`   ⮕ ${afterCondition}`, vscode.TreeItemCollapsibleState.None, 'flag-after');
                        afterItem.iconPath = new vscode.ThemeIcon('arrow-small-right', new vscode.ThemeColor('charts.yellow'));
                        items.push(afterItem);
                    }
                    else {
                        const item = new RegisterTreeItem(`${flag} = ${afterCondition}`, vscode.TreeItemCollapsibleState.None, 'flag');
                ...
                if (frameSlots.length > 0) {
                    ...
                    const divider = fb > 0
                        ? `────────── ${fb} bytes ──────────`
                        : `──────────────────────────────`;
                    const divItem = new RegisterTreeItem(divider, vscode.TreeItemCollapsibleState.None, 'stack-header');
                    ...
                    for (const [addr, afterVal] of frameSlots) {
                        ...
                        if (changed && beforeVal) {
                            const beforeItem = new RegisterTreeItem(`${addr}: ${formatValue(beforeVal)}`, vscode.TreeItemCollapsibleState.None, 'stack');
                            beforeItem.iconPath = new vscode.ThemeIcon('symbol-variable', new vscode.ThemeColor('charts.yellow'));
                            items.push(beforeItem);
                            const afterItem = new RegisterTreeItem(`   ⮕ ${formatValue(afterVal)}`, vscode.TreeItemCollapsibleState.None, 'memory-after');
                            afterItem.iconPath = new vscode.ThemeIcon('arrow-small-right', new vscode.ThemeColor('charts.yellow'));
                            items.push(afterItem);
                        }
                        else {
                            const item = new RegisterTreeItem(`${addr}: ${formatValue(afterVal)}`, vscode.TreeItemCollapsibleState.None, 'stack');
                            item.iconPath = new vscode.ThemeIcon('symbol-variable', changed ? new vscode.ThemeIcon('charts.yellow') : undefined);
                            items.push(item);
                        }
                    }
                }
                ...
                for (const change of changes.memory) {
                    ...
                    items.push(item);
                    const afterItem = new RegisterTreeItem(`   ⮕ ${change.after}`, vscode.TreeItemCollapsibleState.None, 'memory-after');
                    afterItem.iconPath = new vscode.ThemeIcon('arrow-small-right', new vscode.ThemeColor('charts.yellow'));
                    items.push(afterItem);
                }
                // Unchanged memory
                for (const [addr, value] of Object.entries(this.stateAfter.memory)) {
                    if (!changes.memory.some(c => c.name === addr)) {
                        const item = new RegisterTreeItem(`*${addr}`, vscode.TreeItemCollapsibleState.None, 'memory');
    ...
    createRegisterItem(reg, afterValue, changes) {
        ...
        if (change) {
            ...
            const afterItem = new RegisterTreeItem(`   ⮕ ${change.after}`, vscode.TreeItemCollapsibleState.None, 'register-after', reg, afterValue);
            afterItem.iconPath = new vscode.ThemeIcon('arrow-small-right', new vscode.ThemeColor('charts.yellow'));
...
function evaluateBinary(op, left, right) {
    ...
    const imm = (v) => ({ type: 'immediate', value: v });
...
function cleanFrameSlots(state) {
...
function analyzeRegisters(document, targetLine) {
    ...
    for (let i = functionStart; i <= targetLine; i++) {
        ...
        // Handle memory operations
            ...
            const srcBits = srcBitsRaw === 'b' ? 8 : srcBitsRaw === 'w' ? 16 : 32;
            const rawDestReg = operands[1];
            const src = resolveValue(state, parseOperand(operands[0]));
            if (src.type === 'immediate') {
                let signExtended;
